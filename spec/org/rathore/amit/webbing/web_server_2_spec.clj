...
(deftest converting-to-nested-map
  (let [nested (convert-to-nested-map params)]
    (is (map? (nested "d")))
    (let [general-s ((nested "d") "general")
	  general-k ((nested :d) :general)]
      (is (= (general-s "client-time") "1261181538285"))
      (is (= (general-s "request-url") "http://trial-hotel.com/home.html"))
      (is (= (general-s "cid") "encrypted-consumer-id-1261103840540"))
      (is (= (general-s "referrer") ""))
      (is (= (general-s "merchant-session-id") "EBCDIC"))
      (is (= (general-s "timezone-offset") "480"))
      (is (= (general-s "merchant-id") "1c030cab-5cbf-c62c-5a3c-287af5b8822b"))
